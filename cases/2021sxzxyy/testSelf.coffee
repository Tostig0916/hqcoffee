...
class 指标填报表 extends 指标体系
  @saveExcel: (funcOpts={}) ->
    ...
    opts.data = [
      {
        ...
        when (value.院科通 in [1,3]) and /(自|考|监)/.test(value[customGrade])).sort(
          (a,b)-> switch 
    ...
    for 科室名, 科室 of 科室设置 when 科室.内外全 in [1,2,3]
      opts.data.push {
        sheet: 科室名
        columns:[
          {label:'数据名',value:'数据名'}
        ...
        content: (value for key, value of json \
        when (value.院科通 in [1,2,3]) and /(自|考|监)/.test(value[customGrade])).sort(
          (a,b)-> switch 
            when b.上级指标 > a.上级指标 then -1
            when b.上级指标 is a.上级指标 then switch
              when b.数据名 > a.数据名 then -1
...
class 生成器 extends CaseSingleton
  ...
  # 查看各自 db, 以及log
  @showDBs: ->
    console.log {db: v.dbValue()} for k, v of {院内资料库,院科内部分析报告,对标资料库,院科外部对标报告,别名库,项目别名库,MissingDataLog,三级指标对应二级指标,名字ID库,MissingDataFunctions}
    console.log {log: v.logdb?().value()} for k, v of {院内资料库,院科内部分析报告,对标资料库,院科外部对标报告,别名库,项目别名库,MissingDataLog,三级指标对应二级指标,名字ID库}
    return this
